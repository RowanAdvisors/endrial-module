...
export class LootSheetActions {
  ...
  static chatMessage(speaker, owner, message, item) {
    if (game.settings.get("lootsheetnpcsfrpg", "buyChat")) {
      if (item) {
        message = `<div class="sfrpg chat-card item-card" data-actor-id="${owner._id}" data-item-id="${item._id}">
        ...
        message = `<div class="sfrpg chat-card item-card" data-actor-id="${owner._id}">
                    <div class="card-content"><p>${message}</p></div></div>`;
      }
      ChatMessage.create({
        user: game.user._id,
  ...
  static errorMessageToActor(target, message) {
    game.socket.emit("module.lootsheetnpcsfrpg", {
      type: "error",
      targetId: target.id,
      message: message
    });
  ...
  static moveItem(source, destination, itemId, quantity) {
    ...
    if(newItem.flags.lootsheetnpcsfrpg) {
      delete(newItem.flags.lootsheetnpcsfrpg)
    }

    // decrease the quantity (unless infinite)
    if(!item.flags.lootsheetnpcsfrpg || !item.flags.lootsheetnpcsfrpg.infinite) {
      const update = {
        _id: itemId,
        "data.quantity": item.data.quantity - quantity
      };

      let removeEmptyStacks = game.settings.get("lootsheetnpcsfrpg", "removeEmptyStacks");
  ...
  static async dropOrSellItem(speaker, container, giver, itemId) {
    ...
    if(container.getFlag("lootsheetnpcsfrpg", "lootsheettype") === "Merchant") {
  ...
  static async transaction(speaker, seller, buyer, itemId, quantity) {
    ...
    let sellerModifier = seller.getFlag("lootsheetnpcsfrpg", "priceModifier");
